...
export default function SignIn() {
  ...
  const submitLoginRequest = async (accountId?: number | null) => {
    const response = await fetch("/auth/login", {
      ...
      body: JSON.stringify({
        ...
        accountId: accountId ?? selectedAccountId ?? null,
      }),
    });
  ...
  if (step === "login") {
    return (
      <>
        ...
        >
          <DialogContent className="max-w-md rounded-2xl border border-amber-200 bg-white">
            <DialogHeader>
              <DialogTitle className="text-gray-900">
                Choose account
              </DialogTitle>
              <DialogDescription className="text-gray-600">
                This email is linked to more than one account. Pick the one you
                want to open.
              </DialogDescription>
            </DialogHeader>
              ...
              {accountChoices.map((account) => (
                <button
                  ...
                  onClick={async () => {
                    ...
                    const nextData = await submitLoginRequest(account.id);

                    if (nextData.requiresVerification) {
                      setVerificationEmail(nextData.email || formData.email);
                      setStep("verify-sent");
                      return;
                    }

                    if (nextData.requiresOtp) {
                      setOtpToken(nextData.otpToken);
                      setMfaCode(["", "", "", "", "", ""]);
                      setOtpError("");
                      setStep("login-otp");
                      return;
                    ...
                    if (nextData.success && nextData.token && nextData.user) {
                      ...
                      if (redirectTarget && redirectTarget.startsWith("/")) {
                        ...
                        const portals = nextData.user.portals || [];
                        if (
                          portals.includes("community") &&
                          !portals.includes("artist")
                        ) {
                          navigate("/blog");
                        } else {
                          navigate(getLoginDashboardPath(nextData.user));
                        }
                      }
                      return;
                    }
                    ...
                    setLoginError(
                      nextData.message || "Could not sign in to that account.",
                    );
                  }}
                  className="w-full rounded-xl border border-gray-200 bg-gray-50 p-3 text-left transition hover:border-[#bf831c] hover:bg-amber-50"
                >
                  ...
                  <div className="mb-6 p-4 bg-orange-50 border border-orange-200 rounded-lg flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-orange-700 text-sm">
                        Try logging in — you'll be prompted to resend a new
                        link.
                      </p>
                    </div>
                  </div>
