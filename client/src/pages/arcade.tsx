...
export default function ArcadePage() {
  ...
  // GOD TIER: CEO, superadmin, admin_001 — full access, highest subscription, never charged
  const isGodTier =
    user?.username === "admin_001" ||
    user?.username === "superadmin" ||
    user?.username === "CEO" ||
    user?.username === "admin_025" || // CEO account
    user?.email === "admin@versoair.com" ||
    user?.role === "admin" ||
    user?.role === "superadmin" ||
    user?.role === "ceo" ||
    String(user?.id) === "1"; // First user is typically admin
  ...
  const artistTier =
    ...
    (isGodTier ? "boss" : "free"); // God tier gets "boss" subscription
  const artistNeedsUpgrade = isArtist && !isGodTier && artistTier === "free";
  ...
  // ── Persistent back button (portal to body so it escapes the <main> opacity wrapper) ──
  const backButton = createPortal(
    <button
      onClick={() => window.history.back()}
      className="fixed bottom-6 left-4 z-[10000] p-2 rounded-lg bg-white/10 hover:bg-white/20 backdrop-blur-md border border-white/10 hover:border-white/20 transition-all duration-200 text-white pointer-events-auto"
      title="Go back"
    >
        ...
        <div className="min-h-screen bg-[#06020f] flex items-center justify-center p-4">
          <Card className="bg-black/40 border-amber-500/30 max-w-md w-full text-center p-8">
            ...
            <p className="text-gray-400 mb-2">
              L'Arcade est incluse dans votre abonnement artiste à partir du
              tier{" "}
              <span className="text-blue-400 font-semibold">Essential</span>.
            </p>
            <p className="text-gray-500 text-sm mb-6">
              Les auditeurs et streamers y accèdent gratuitement — en tant
              qu'artiste, passez au niveau supérieur pour débloquer les duels
              PvP et les mini-jeux.
            </p>
            <div className="flex flex-col gap-3">
              <Button
        ...
        <div className="min-h-screen bg-[#06020f] flex items-center justify-center p-4">
          <Card className="bg-black/40 border-purple-500/30 max-w-md w-full text-center p-8">
            ...
            <p className="text-gray-400 mb-2">
              ...
              <span className="text-purple-400 font-semibold">Champion</span>,
              ou <span className="text-purple-400 font-semibold">Patron</span>.
            </p>
            <p className="text-gray-500 text-sm mb-6">
              ...
              <div>
                ...
                {!isArtist && (
                  <Badge className="mt-2 bg-purple-600/20 text-purple-300 border-purple-500/30 text-xs">
                    ...
                    Accès{" "}
                    {streamerTier.charAt(0).toUpperCase() +
                      streamerTier.slice(1)}
                  </Badge>
                )}
              </div>
            ...
            <TabsContent value="play" className="space-y-6">
              <AnimatePresence mode="wait">
                ...
                currentQuestion ? (
                  ...
                  <motion.div key="game" {...fadeInUp}>
                    <Card className="bg-black/50 border-purple-500/30">
                      <CardHeader className="text-center">
                        ...
                        <Progress
                          value={
                            (countdown / (currentQuestion.time_limit || 20)) *
                            100
                          }
                          className="mt-2 h-2"
                        />
                  ...
                  <motion.div key="waiting" {...fadeInUp}>
                    <Card className="bg-black/50 border-purple-500/30 text-center py-12">
                      ...
                      <p className="text-gray-400 mb-1">
                        Match #{activeMatch.id} • Mise:{" "}
                        {activeMatch.wager_amount} crédits
                      </p>
                  ...
                  /* ── Create Challenge ── */
                  <motion.div key="create" {...fadeInUp} className="space-y-6">
                      ...
                      <CardContent className="flex items-center justify-between py-4">
                        ...
                        <div className="text-right">
                          <p className="text-amber-400 font-bold text-lg">
                            {parseFloat(entry.total_winnings).toLocaleString()}{" "}
                            cr
                          </p>
                        </div>
                      </CardContent>
