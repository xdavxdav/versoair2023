...
export default function GeoAdminAuthGate({
  ...
  // ─── Create .test account floating panel ────────────────────────────────────
  const [showCreateTest, setShowCreateTest] = useState(false);
  const [createEmail, setCreateEmail] = useState("");
  const [createGateUsername, setCreateGateUsername] = useState("");
  ...
  // Auto-fill @versoair.test domain as user types
  const handleCreateEmailChange = (val: string) => {
    setCreateEmail(val);
    // Auto-derive gate username from email prefix
    const prefix = val.split("@")[0];
    if (prefix && !createGateUsername) {
      setCreateGateUsername(prefix.toLowerCase().replace(/[^a-z0-9_]/g, "_"));
    }
  };
  ...
  const handleCreateTestAccount = async (e: React.FormEvent) => {
    ...
    // Client-side .test enforcement
    if (!createEmail.endsWith(".test")) {
      setCreateError("Email must use a .test domain (e.g. name@versoair.test)");
      return;
    }

    setCreateLoading(true);
    try {
      const res = await fetch("/auth/register-geoadmin", {
        method: "POST",
        ...
        body: JSON.stringify({
          ...
          gateUsername: createGateUsername.toLowerCase().trim(),
        }),
      });
      ...
      if (data.success && data.token) {
        ...
        localStorage.setItem(
          "geoadmin_username",
          createGateUsername.toLowerCase(),
        );
        ...
        setTimeout(() => {
          onSignInSuccess(createGateUsername.toLowerCase());
        }, 1200);
      } else {
        setCreateError(data.message || "Registration failed.");
      }
    } catch {
  ...
  if (mode === "gate") {
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-indigo-950 relative overflow-hidden">
        ...
        {/* ─── Floating "Create .test" button + panel ─────────────────────────── */}
        <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end gap-3">
          {/* Expandable registration panel */}
          {showCreateTest && (
            <div className="w-80 bg-slate-900/95 border border-white/10 rounded-2xl shadow-2xl shadow-black/40 p-5 animate-in fade-in slide-in-from-bottom-2 duration-300">
              {createSuccess ? (
                ...
                <>
                  ...
                  <form
                    onSubmit={handleCreateTestAccount}
                    className="space-y-3"
                  >
                    <div>
                      ...
                      <div className="relative">
                        ...
                        <Input
                          ...
                          value={createEmail}
                          onChange={(e) =>
                            handleCreateEmailChange(e.target.value)
                          }
                          placeholder="name@versoair.test"
                          required
                          className="pl-8 h-9 text-xs bg-white/5 border-white/10 text-white placeholder:text-slate-600 focus:border-emerald-500/50"
                        />
                      </div>
                    </div>
                    ...
                    <div>
                      ...
                      <div className="relative">
                        ...
                        <Input
                          type="text"
                          value={createGateUsername}
                          onChange={(e) =>
                            setCreateGateUsername(
                              e.target.value
                                .toLowerCase()
                                .replace(/[^a-z0-9_]/g, ""),
                            )
                    ...
                    <div>
                      ...
                      <div className="relative">
                        ...
                        <Input
                          ...
                          required
                          className="pl-8 pr-8 h-9 text-xs bg-white/5 border-white/10 text-white placeholder:text-slate-600 focus:border-emerald-500/50"
                        />
                        <button
                          type="button"
                    ...
                    <Button
                      type="submit"
                      disabled={createLoading || !createEmail.endsWith(".test")}
                      className="w-full h-9 text-xs bg-gradient-to-r from-emerald-600 to-emerald-700 hover:from-emerald-700 hover:to-emerald-800 text-white rounded-lg disabled:opacity-40"
                    >
                      {createLoading ? (
                        <>
                    ...
                    <p className="text-[10px] text-slate-600 text-center leading-tight">
                      Only <span className="text-emerald-400/60">.test</span>{" "}
                      domains accepted. No .com emails for GeoAdmin accounts.
                    </p>
                  </form>
                </>
              )}
          ...
          {!showCreateTest && (
            <button
              ...
              className="group flex items-center gap-2 px-4 py-2.5 bg-slate-800/80 hover:bg-slate-700/90 border border-white/[0.08] hover:border-emerald-500/30 rounded-full shadow-lg shadow-black/30 transition-all duration-300 hover:shadow-emerald-500/10"
            >
              <Shield className="h-3.5 w-3.5 text-emerald-400/70 group-hover:text-emerald-400 transition-colors" />
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-indigo-950 flex items-center justify-center relative overflow-hidden">
      ...
      {/* ─── Floating "Create .test" on sign-in view too ─────────────────────── */}
      <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end gap-3">
        {showCreateTest && (
          <div className="w-80 bg-slate-900/95 border border-white/10 rounded-2xl shadow-2xl shadow-black/40 p-5 animate-in fade-in slide-in-from-bottom-2 duration-300">
            {createSuccess ? (
              ...
              <>
                ...
                <form onSubmit={handleCreateTestAccount} className="space-y-3">
                  <div>
                    <label className="block text-[10px] font-medium text-slate-500 mb-1 uppercase tracking-wider">
                      Email (.test only)
                    </label>
