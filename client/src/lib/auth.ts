...
export async function authenticatedFetch(
  ...
  // Send Authorization header from in-memory token only.
  // httpOnly cookie (credentials: "include") is the persistent auth fallback.
  if (!headers["Authorization"] && _authToken) {
    headers["Authorization"] = `Bearer ${_authToken}`;
  }
