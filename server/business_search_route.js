    ...
    try {
      ...
      if (category) {
        ...
        if (categoryRecord.length > 0) {
          conditions.push(sql`${businesses.categoryId} = ${categoryRecord[0].id}`);
        }
      }
      ...
      if (conditions.length > 0) {
        sqlQuery = sqlQuery.where(sql`${conditions.reduce((acc, cond) => sql`${acc} AND ${cond}`)}`);
      }
      ...
      const totalInDatabase = await db.select({ count: sql<number>`count(*)` }).from(businesses);
