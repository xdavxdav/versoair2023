...
const pool = new Pool({
  connectionString:
    ...
    "postgresql://postgres:postgres@localhost:5432/versoair_business_intelligence",
});
...
async function generateReport() {
  try {
    ...
    businesses.forEach((biz) => {
      ...
      groupedByCategory[catName].push(biz);
    });
    ...
    const emptyCategories = categories.filter(
      (cat) => !businesses.some((biz) => biz.category_id === cat.id),
    );
    if (emptyCategories.length > 0) {
      report += `### 🟡 Empty Categories (${emptyCategories.length})\n`;
      emptyCategories.forEach((cat) => {
        report += `- ${cat.name} (ID: ${cat.id}, Slug: ${cat.slug})\n`;
      });
      report += `\n`;
    } else {
    ...
    console.log(
      `📊 Found ${businesses.length} businesses across ${categories.length} categories`,
    );
